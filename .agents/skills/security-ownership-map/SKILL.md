...
description: "Security-oriented ownership and bus-factor analysis grounded in git history. Use when the user explicitly wants orphaned sensitive code, CODEOWNERS reality checks, sensitive hotspots, security maintainers, or ownership-cluster analysis. Do not trigger for general maintainer lists or non-security ownership questions."
---
...
3. Build the ownership map with `scripts/build_ownership_map.py` or `scripts/run_ownership_map.py` (co-change graph is on by default; use `--cochange-max-files` to ignore supernode commits).
...
```bash
python .agents/skills/security-ownership-map/scripts/run_ownership_map.py \
  --repo /path/to/linux \
...
Use `scripts/query_ownership.py` to return small, JSON-bounded slices without loading the full graph into context. The main repo-local scripts you will normally use are:
...
```bash
python .agents/skills/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out people --limit 10
python .agents/skills/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out files --tag auth --bus-factor-max 1
python .agents/skills/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out person --person alice@corp --limit 10
python .agents/skills/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out file --file crypto/tls
python .agents/skills/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out cochange --file crypto/tls --limit 10
python .agents/skills/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out summary --section orphaned_sensitive_code
python .agents/skills/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out community --id 3
```
