    ...
    while period_cursor <= period_end_anchor:
        ...
        if args.window_days > 0:
            ...
            def in_bucket(
                ...
                end: dt.datetime = bucket_end,
            ) -> bool:
                return start <= commit_date <= end
        else:
            if args.bucket == "quarter":
                ...
                def in_bucket(
                    commit_date: dt.datetime,
                    start: dt.datetime = bucket_start,
                    end: dt.datetime = bucket_end,
                ) -> bool:
                    return start <= commit_date <= end
            else:

                def in_bucket(commit_date: dt.datetime, end: dt.datetime = bucket_end) -> bool:
                    return commit_date.year == end.year and commit_date.month == end.month
